...
export interface ProLinkBody { initData?: string; plan?: unknown; chatId?: number; }
...
/** BirthdayBot's Pro is per-GROUP (`groups.pro`), and this Mini App aggregates every group
 * a user belongs to — there is no single bot-wide `pro` boolean, so the generic single
 * `#pro`/renderPro block from webapp-i18n.ts is not used here. Each group section renders
 * its own "Unlock Pro" row (via `buyProFor(chatId, btn)` below) when that group is not Pro
 * yet; `#pro` stays an empty, unused container kept only because the spec requires it. */
...
h1{font-size:18px;margin:0 0 12px}h2.grp{font-size:14px;margin:16px 0 6px;color:var(--tg-theme-hint-color,#777)}
.h{display:flex;align-items:center;gap:10px;padding:12px;border-radius:12px;background:var(--tg-theme-secondary-bg-color,#f3f3f3);margin-bottom:8px}
...
.empty{color:var(--tg-theme-hint-color,#777)}.unlock{width:100%;margin:2px 0 10px;font-size:13px;padding:8px 12px}button[disabled]{opacity:.5}</style></head><body>
...
function renderMore(){const h='<h2 class="grp">'+T('app_moreApps')+'</h2>';document.getElementById('more').innerHTML=h+OTHER_BOTS.map(([label,bot])=>'<button class="mo" data-bot="'+bot+'">'+label+'</button>').join('');for(const b of document.querySelectorAll('.mo'))b.onclick=()=>tg.openTelegramLink('https://t.me/'+b.dataset.bot)}
...
function renderShare(){const el=document.getElementById("shareRow");if(!el)return;let ok=false;try{ok=typeof tg.shareMessage==="function"&&tg.isVersionAtLeast("8.0")}catch(e){}
 ...
 if(ok2){const s=sBtn(T('app_shareStory'),"8px");s.onclick=()=>{try{api("/api/share-story",{}).catch(()=>{});tg.shareToStory("https://tg.zovo.one/img/banner-birthday.png",{text:T('app_storyText')+"\\n\\n${STORY_LINK}",widget_link:{url:"${STORY_LINK}",name:"${TITLE}"}})}catch(e){}};el.appendChild(s)}}
/** Per-group Stars checkout: this bot's Pro is scoped to one group, so unlike the shared
 * buyPro(plan,btn) from webapp-i18n.ts, this one carries the target chatId through to
 * POST /api/pro-link and refreshes the whole board (load()) once the invoice resolves. */
async function buyProFor(chatId,btn){btn.disabled=true;
 try{const d=await api('/api/pro-link',{plan:'onetime',chatId:chatId});
  if(!d||!d.url){btn.disabled=false;if(!d||!d.qa)note(T('app_payFailed'));return}
  tg.openInvoice(d.url,function(st){btn.disabled=false;
   if(st==='paid'){note(T('app_payDone'));load()}else if(st==='cancelled'){note(T('app_payCancelled'))}else{note(T('app_payFailed'))}})}
 catch(e){btn.disabled=false;note(T('app_payFailed'))}}
function birthdayRow(u){const row=document.createElement('div');row.className='h';
 const b=document.createElement('b');b.textContent=u.name;b.appendChild(document.createElement('br'));
 const small=document.createElement('small');small.textContent=u.label;b.appendChild(small);row.appendChild(b);return row}
function groupSection(g){const frag=document.createDocumentFragment();
 const h=document.createElement('h2');h.className='grp';h.textContent=(g.title||T('app_group'))+(g.pro?' · Pro':'');frag.appendChild(h);
 if(!g.pro&&typeof tg.openInvoice==='function'){const ub=document.createElement('button');ub.className='unlock';
  ub.textContent=T('app_unlockPro')+' — '+T('app_proOneTime',{n:d_proStars});
  ub.onclick=()=>buyProFor(g.chat_id,ub);frag.appendChild(ub)}
 if(!g.upcoming.length){const e=document.createElement('div');e.className='empty';e.textContent=T('app_empty');frag.appendChild(e);return frag}
 for(const u of g.upcoming)frag.appendChild(birthdayRow(u));
 return frag}
let d_proStars=0;
async function load(){let d;try{d=await api('/api/upcoming',{})}catch(e){d=null}
 const el=document.getElementById('list');
 if(!d||d.error){el.className='empty';el.textContent=errText(d);return}
 d_proStars=d.proStars||0;note('');
 if(!d.groups.length){el.className='empty';el.textContent=T('app_empty');return}
 el.className='';el.innerHTML='';for(const g of d.groups)el.appendChild(groupSection(g))}
